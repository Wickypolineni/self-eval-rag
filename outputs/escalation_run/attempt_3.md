...
RAG helps address these problems by giving the LLM access to fresh information. It reduces hallucination and makes answers more up-to-date, but it does not eliminate errors—the model can still misread or contradict the retrieved text. RAG works at the time you ask a question; it does not change what the LLM has learned before. It is like giving your friend a new book to read *right now* to answer your question, instead of teaching them the book's contents forever. While a special database called a vector store is often used, it is not always needed for RAG.

This means the LLM can give you:

*   **More Correct Answers**: It uses real information, not guesses.
...
This whole process—from preparing the library to giving you an answer—is how RAG helps LLMs be smarter and more reliable.